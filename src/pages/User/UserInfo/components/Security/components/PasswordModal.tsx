...
const PasswordModal: FC<PasswordModalProps> = ({visible, onCancel}) => {
  ...
  const checkOldPassword = (_: any, value: string) => {
    ...
    //有值的情况
    if (value.length < 6) {
      return promise.reject('密码至少为8个字符！');
    }
    return promise.resolve();
  }
  ...
  const updatePassword = async () => {
    ...
    if (oldPassword.length >= 8 && password.length >= 8 && password === confirm) {
      const params = {
        originalPassword: oldPassword,
        newPassword: password
      }
      const res = await updatePasswordUsingPost(params);
      if(res.code === 0){
      ...
      <Form labelCol={{ span: 4 }} wrapperCol={{ span: 20 }} form={form} name="UserRegister" onFinish={onFinish}>
        ...
        >
            <FormItem
              ...
              required={true}
            >
              <Input type="password" placeholder="至少8位密码，区分大小写" />
            </FormItem>
          </Popover>
          <FormItem
            name="confirm"
