...
import {createStarBookUsingPost, getStarBooksUsingGet} from "@/services/backend/starBookController";
import {starGeneratorUsingPost} from "@/services/backend/generatorStarController";
...
const StarModal: React.FC<StarModalProps> = ({generatorId, visible, onCancel, onOver}) => {
  const [buttonVisible, setButtonVisible] = useState(true);
  const [starBookName, setStarBookName] = useState('');
  const inputRef = useRef(null);
  const [starBookBools, setStarBookBools] = useState<API.StarBookBoolVo[]>([]);
  const [selectIds, setSelectIds] = useState<number[]>([]);
  const [idsStr, setIdsStr] = useState<string>('');

  //查询该用户的所有收藏夹信息
  useEffect(() => {
    getStarBooksUsingGet({generatorId}).then(res => {
      if(res.code === 0 && res.data){
        setStarBookBools(res.data);
        const ids = res.data.filter((starBookBool: API.StarBookBoolVo) => starBookBool.isContain)
          .map((starBookBool:API.StarBookBoolVo) => starBookBool.id);
  ...
  const addToStarBook = () => {
    starGeneratorUsingPost({id: generatorId},selectIds).then(res => {
      if(res.code === 0 && res.data){
        switch (res.data){
  ...
  const handleInputConfirm = (event: any) => {
    ...
    if(StringUtils.isNotEmpty(starBookName)){
      createStarBookUsingPost({name: starBookName}).then(res => {
        if(res.code === 200){
